    ...
    '<div class="data-obj-list">',
        '{{#each models}}',
            '<div class="privilege-entry row-fluid">',
                ...
                '<div class="span10">',
                    //Data api - self
                    '<div><span class="entry-type">Data Access: </span>',
                    '<div class="btn-group" data-toggle="buttons-checkbox">{{#each routes.data.std}}<button type="button" class="btn">{{@key}}</button>{{/each}}</div>',
                    '</div>',
                    //Data api - ref [per field]
                    '<div><span class="entry-type">Reference Data Access: </span>',
                        '{{#each routes.data.ref}}',
                            '<div>',
                                '<span class="label label-info field-label">{{@key}}</span> ',
                                '<div class="btn-group" data-toggle="buttons-checkbox">{{#each this}}<button type="button" class="btn">{{@key}}</button>{{/each}}</div>',
                            '</div>',
                        '{{else}}',
                    ...
                    '</div>',
                    //File api - [per file]
                    '<div><span class="entry-type">File Access: </span>',
                        '{{#each routes.file}}',
                            '<div>',
                                '<span class="label label-info field-label">{{@key}}</span> ',
                                '<div class="btn-group" data-toggle="buttons-checkbox">{{#each this}}<button type="button" class="btn">{{@key}}</button>{{/each}}</div>',
                    ...
                    '<div><span class="entry-type">Logic Access: </span>',
                        '{{#each routes.logic}}',
                            '<div>',
                                '<span class="label label-info field-label">{{@key}}</span> ',
                                '<div class="btn-group" data-toggle="buttons-checkbox"><button type="button" class="btn">exec</button></div>',
                            '</div>',
                        '{{else}}',
                            '<span class="label label-inverse">N/A</span>',
                        '{{/each}}',
